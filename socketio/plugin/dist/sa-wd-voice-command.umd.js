(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.SAWDVoiceCommand = factory());
	...
	class SAWDVoiceCommand {
	    ...
	    record() {
	        if (this.stream) {
	            this.recorder = new RecordRTCExports.StereoAudioRecorder(this.stream, {
	                type: 'audio',
	                mimeType: 'audio/wav',
	                numberOfAudioChannels: 1
	            });
	        ...
	        this.xhr('/asr?method=openai-whisper&task=transcribe&output=json&language=ru&encode=true', formData, (response) => {
	            ...
	            fetch('/text', {
	                ...
	                resPython.json().then(text => {
	                    fetch('/api/tts?voice=' + encodeURIComponent('ru_RU/multi_low#hajdurova') + '&noiseScale=0.667&noiseW=0.8&lengthScale=1&ssml=false&audioTarget=client&text=' + encodeURIComponent(text.resp)).then((res) => {
